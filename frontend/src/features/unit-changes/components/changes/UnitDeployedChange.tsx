...
export interface UnitDeployedChangeProps {
  change: Change;
  playerName: string;
  timeDisplay: string;
...
export const UnitDeployedChange: React.FC<UnitDeployedChangeProps> = ({
  change,
  playerName,
  timeDisplay,
  heroesData
    ...
    <div className="unit-changes-widget__change-info">
      ...
      {change.position && (
        <UIText variant="label" className="unit-changes-widget__unit-position">
          Deployed at: ({change.position.x}, {change.position.y})
        </UIText>
      )}
    </div>
  );
