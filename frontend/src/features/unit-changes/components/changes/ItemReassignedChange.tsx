...
export interface ItemReassignedChangeProps {
  change: Change;
  playerName: string;
  timeDisplay: string;
...
export const ItemReassignedChange: React.FC<ItemReassignedChangeProps> = ({
  change,
  playerName,
  timeDisplay,
  heroesData,
  ...
  // Try to find the units involved
  const previousUnit = player?.units?.find(
    (u: any) => u.entindex === change.previous_assigned_unit_entindex
  );
  const newUnit = player?.units?.find(
    (u: any) => u.entindex === change.new_assigned_unit_entindex
  );
  ...
  const content = (
    <div className="unit-changes-widget__item-change-info">
      ...
      {change.previous_assigned_unit_entindex !== null && change.new_assigned_unit_entindex !== undefined && (
        <div className="unit-changes-widget__item-assignment">
          ...
          {newUnit && heroesData ? (
            ...
            <UIText variant="label" className="unit-changes-widget__assignment-text">
              Unit {change.new_assigned_unit_entindex}
            </UIText>
          )}
        </div>
      )}
