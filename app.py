...
# GSI endpoint is fixed by game configuration91968975
...
PRIVATE_PLAYER_ACCOUNT_ID = "91968975"
...
class MatchState:
    def __init__(self):
        ...
        # NEW: Round tracking state machine
        self.tracked_player_id = None  # The "source of truth" player
...
def get_highest_hp_player(players: dict) -> str:
    ...
    for player_id, player_data in players.items():
        ...
        if hp > max_hp:
            max_hp = hp
            highest_hp_player = player_id

    return highest_hp_player


def update_round_from_combat_type(player_id: str, combat_type: int):
    """Update round number and phase based on combat_type transitions."""

    # Initialize tracked player if not set or if current tracked player is gone
    if (
        not match_state.tracked_player_id
        or match_state.tracked_player_id not in match_state.players
    ):
        match_state.tracked_player_id = get_highest_hp_player(match_state.players)
            ...
            if isinstance(task, tuple) and len(task) >= 2 and isinstance(task[0], str):
                ...
                if task_type == "insert_snapshot":
                    ...
                    if player_id == "private_player":
                        ...
                        db.insert_public_snapshot(
                            match_id, player_id, player_data, timestamp
                        )
                    ...
                    db.update_player_final_place(
                        match_id, player_id, final_place, timestamp
                    )
                    ...
                    try:
                        ...
                        db.update_match_end_time(match_id, timestamp)
                        print(
                            f"[DB Writer] Match end transaction completed for {match_id}"
                        )
                ...
                print(
                    f"[DB Writer] Expected format: (task_type, ...) where task_type is a string"
                )
...
def is_valid_new_player(public_state):
    ...
    return (
        public_state.get("health") == 100
        and public_state.get("level", 0) == 1
        and public_state.get("wins", 0) == 0
        and public_state.get("losses", 0) == 0
        and public_state.get("xp", 0) == 0
    )
...
def check_match_end(match_id: str, timestamp: datetime) -> bool:
    ...
    if second_place:
        ...
        if not winner and still_playing:
            ...
            print(
                f"[MATCH END] Player {second_place[0]['player_id']} got 2nd, marking {winner_id} as winner"
            )
            ...
            try:
                ...
                db_write_queue.put(
                    ("match_end_transaction", match_id, winner_id, timestamp)
                )
    ...
    if len(still_playing) == 0 and len(players) > 0:
        print(
            f"[MATCH END] All {len(players)} players have final places - ending match"
        )
...
def abandon_match(match_id: str, timestamp: datetime, reason: str = "Manual"):
    ...
    socketio.emit(
        "match_abandoned",
        {"match_id": match_id, "reason": reason, "timestamp": timestamp.isoformat()},
        to=None,
    )
...
def update_public_player_state(player_id: str, public_state: Dict, timestamp: datetime):
    ...
    player_state = {
        ...
        "persona_name": existing_player.get("persona_name")
        or public_state.get("persona_name"),
        "bot_persona_name": existing_player.get("bot_persona_name")
        or public_state.get("bot_persona_name"),
        "player_slot": existing_player.get("player_slot")
        or public_state.get("player_slot", 0),
        ...
        "underlord_selected_talents": public_state.get(
...
def update_private_player_state(private_state: Dict, timestamp: datetime):
    ...
    private_player_state = {
        ...
        "used_item_reward_reroll_this_round": private_state.get(
            "used_item_reward_reroll_this_round", False
        ),
...
def emit_realtime_update():
    ...
    update_data = {
        "match": {
            ...
            "started_at": (
                match_state.match_start.isoformat() if match_state.match_start else None
            ),
...
def process_buffered_data(match_id: str, confirmed_players: set, timestamp: datetime):
    ...
    for buf_state, buf_time in match_state.buffer:
        ...
        if buf_player_id in confirmed_players:
            if (
                buf_player_id not in latest_sequences
                or buf_sequence > latest_sequences[buf_player_id]
            ):
    ...
    for buf_state, buf_time in match_state.buffer:
        ...
        if (
            buf_player_id in confirmed_players
            and buf_player_id in latest_sequences
            and buf_sequence == latest_sequences[buf_player_id]
        ):
            ...
            db_write_queue.put(
                ("insert_snapshot", match_id, buf_player_id, buf_state, buf_time)
            )
        ...
        for block in data["block"]:
            ...
            for data_obj in block["data"]:
                ...
                if "private_player_state" in data_obj:
                    ...
                    if (
                        "private_sequence" in match_state.sequences
                        and match_state.sequences["private_sequence"]
                        >= private_sequence_num
                    ):
                        continue
                ...
                if (
                    player_id in match_state.sequences
                    and match_state.sequences[player_id] >= sequence_num
                ):
                    continue
                ...
                if match_state.match_id is None:
                    if (
                        is_valid_new_player(public_state)
                        and player_id not in match_state.candidates
                    ):
                        match_state.candidates[player_id] = public_state
                        print(
                            f"[CANDIDATE] Player {player_id} ({len(match_state.candidates)}/8)"
                        )
                    ...
                    if len(match_state.candidates) == 8:
                        ...
                        match_players_data = [
                            match_state.candidates[p] for p in confirmed
                        ]
                    ...
                    if (
                        player_id == PRIVATE_PLAYER_ACCOUNT_ID
                        and player_id in match_state.players
                    ):
                        ...
                        if (new_health == 100 and old_health < 100) or (
                            new_slot != old_slot and new_slot > 0
                        ):
                            print(
                                f"[MATCH ABANDONED] Detected new game start (health: {old_health}→{new_health}, slot: {old_slot}→{new_slot})"
                            )
                            abandon_match(
                                match_state.match_id,
                                timestamp,
                                reason="Client owner started new game",
                            )
            ...
            "active_match": (
                {
                    "match_id": match_state.match_id,
                    "started_at": (
                        match_state.match_start.isoformat()
...
def get_matches():
    ...
    return jsonify({"status": "success", "matches": matches, "count": len(matches)})
...
def delete_match(match_id):
    ...
    if success:
        return jsonify(
            {"status": "success", "message": f"Match {match_id} deleted successfully"}
        )
    else:
...
def handle_connect():
    ...
    if match_state.match_id:
        print(
            f"[WebSocket] Sending current game state to new client: {match_state.match_id}"
        )
        emit_realtime_update()
